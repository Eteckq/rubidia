...
public class DamageManager {
	...
	public static void damage(LivingEntity damaged, LivingEntity damager, double damages, RDamageCause cause){
		...
		RubidiaEntityDamageEvent e = new RubidiaEntityDamageEvent(damaged, damager, null, damages);
	...
	public static void damageEvent(EntityDamageByEntityEvent event, double damages, RDamageCause cause, boolean critical){
		...
		RubidiaEntityDamageEvent e = new RubidiaEntityDamageEvent(damaged, damager, projectile, damages);
		Bukkit.getPluginManager().callEvent(e);
		if(!e.isCancelled()){
						...
						if(weapon != null){
							if(weapon.isAttack()){
								...
								if(cause.equals(RDamageCause.MELEE)){
									...
									if(rp != null)factor += rp.getAgility()*Settings.AGILITY_FACTOR_RANGE_DAMAGES_ON_RANGE;
