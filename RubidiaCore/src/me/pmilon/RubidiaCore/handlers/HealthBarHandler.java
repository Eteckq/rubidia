...
public class HealthBarHandler implements Listener {
	...
	public static String getTheoricName(RPlayer rp, LivingEntity entity){
		if(entity != null){
			...
			if(entity instanceof Bat)name = rp.translateString("BAT", "CHAUVE-SOURIS");
			else if(entity instanceof CaveSpider)name = rp.translateString("CAVE SPIDER", "ARAIGNEE BLEUE");
			else if(entity instanceof Chicken)name = rp.translateString("CHICKEN", "POULE");
			else if(entity instanceof Cow)name = rp.translateString("COW", "VACHE");
			else if(entity instanceof Giant)name = rp.translateString("GIANT", "GEANT");
			else if(entity instanceof Guardian)name = rp.translateString("GUARDIAN", "GARDIEN");
			else if(entity instanceof Horse)name = rp.translateString("HORSE", "CHEVAL");
			else if(entity instanceof IronGolem)name = rp.translateString("IRON GOLEM", "GOLEM DE FER");
			else if(entity instanceof MagmaCube)name = rp.translateString("MAGMA CUBE", "CUBE DE MAGMA");
			else if(entity instanceof MushroomCow)name = rp.translateString("MUSHROOM COW", "CHAMPIMEUH");
			else if(entity instanceof Pig)name = rp.translateString("PIG", "COCHON");
			else if(entity instanceof PigZombie)name = rp.translateString("PIG ZOMBIE", "COCHON ZOMBIE");
			else if(entity instanceof Rabbit)name = rp.translateString("RABBIT", "LAPIN");
			else if(entity instanceof Sheep)name = rp.translateString("SHEEP", "MOUTON");
			else if(entity instanceof Skeleton)name = rp.translateString("SKELETON", "SQUELETTE");
			else if(entity instanceof Snowman)name = rp.translateString("SNOW GOLEM", "GOLEM DE NEIGE");
			else if(entity instanceof Spider)name = rp.translateString("SPIDER", "ARAIGNEE");
			else if(entity instanceof Squid)name = rp.translateString("SQUID", "PIEUVRE");
			else if(entity instanceof Villager)name = rp.translateString("VILLAGER", "VILLAGEOIS");
			else if(entity instanceof Witch)name = rp.translateString("WITCH", "SORCIERE");
			else if(entity instanceof Wolf)name = rp.translateString("WOLF", "LOUP");
