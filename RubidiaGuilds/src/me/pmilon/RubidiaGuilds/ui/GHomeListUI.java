				...
				if(e.isLeftClick()){
					...
					if(slot < this.getGuild().getLevel()){
						if(gm.canSetHome(slot)){
							...
							rp.sendMessage("§aGet to the desired home and take a representative item to show in your guild homes' list. Then enter the home's name in chat.", "§aDéplacez-vous vers le PR souhaité et prenez dans vos mains un item représentatif qui sera affiché dans la liste des PR de votre guilde. Enfin, entrez dans le chat le nom de votre PR.");
						}else rp.sendMessage("§cYou don't have permission to set homes for your guild!", "§cVous n'avez pas la permission de définir des PR pour votre guilde !");
					...
					if(slot < this.getGuild().getLevel()){
						if(gm.canSetHome(slot)){
							this.getGuild().getHomes()[slot] = null;
							this.getMenu().setItem(slot, this.getDefault(slot));
