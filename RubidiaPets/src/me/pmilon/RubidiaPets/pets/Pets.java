...
public class Pets {
		...
		if(Configs.getPetsConfig().contains("pets")){
			for(String uuid : Configs.getPetsConfig().getConfigurationSection("pets").getKeys(false)){
				...
				Pet pet = new Pet(uuid,
						...
						DyeColor.valueOf(Configs.getPetsConfig().getString("pets." + uuid + ".wolf.collarColor")),
						...
						Ocelot.Type.valueOf(Configs.getPetsConfig().getString("pets." + uuid + ".ocelot.type")),
	...
	public static void save(boolean debug){
		for(Pet pet : pets){
			...
			Configs.getPetsConfig().set(path + ".ocelot.type", pet.getOcelotType().toString());
