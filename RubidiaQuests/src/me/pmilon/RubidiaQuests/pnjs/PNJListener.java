				...
				for(Villager villager : PNJManager.pnjTemps.get(player)){
					...
					if(distance > 1.75){
						if(distance < 36){
							...
							PathEntity pathEntity = v.getNavigation().a(location.getX(), event.getTo().getY(), location.getZ());
							v.getNavigation().a(pathEntity, .6+distance*.03);
			...
			if(objective.getType().equals(ObjectiveType.FOLLOW)){
				...
				if(handler != null){
					...
					if(villager != null){
						...
						new PNJDialog(rp.getPlayer(), handler, villager, objective.getDialogs(), DialogType.POSTDIALOG, new Runnable(){
							...
							if(!DialogManager.isInDialog(p)){
								...
								if(activeQuest != null){
									...
									if(activeQuest.hasFinished(rp)){
										...
										PNJDialog dialog = new PNJDialog(p, handler, villager, activeQuest.getPostDialogs(), DialogType.POSTDIALOG, new Runnable(){
											public void run(){
												activeQuest.finish(rp);
											}
										}, keep, true);
										dialog.start();
									}else new PNJDialog(p, handler, villager, activeQuest.getNonDialogs(), DialogType.NONFINISHEDDIALOG, null, false, true).start();
								}else{
										...
										for(final Quest quest : quests){
											...
											if(objective != null){
												if(!objective.getDialogs().isEmpty()){
													...
													new PNJDialog(p, handler, villager, objective.getDialogs(), DialogType.NONDIALOG, new Runnable(){
