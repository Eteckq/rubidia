		...
		if(rp != null){
			if(rp.fiance != null){
				...
				if(rpp.fiance == rp){
						...
						if(pp.getWorld().equals(villager.getWorld())){
							if(pp.getLocation().distanceSquared(villager.getLocation()) <= Math.pow(8, 2)){
								if(!this.isTaken()){
									...
									new PNJDialog(p, this, villager, Arrays.asList("%player ! Enchanté.", "J'ai entendu parler de vos récentes fiançailles.", "Vous venez donc ici dans le but de vous marier ?", "Bien. Tout est prêt, nous pouvons commencer la cérémonie..."), DialogType.POSTDIALOG, new Runnable(){
										...
										public void run() {
											new BukkitTask(QuestsPlugin.instance){
												...
												@Override
												public void run() {
													PNJDialog dialog1 = new PNJDialog(p, instance, villager, speech, DialogType.PREDIALOG, new Runnable(){
									...
									new PNJDialog(pp, this, villager, Arrays.asList("%player ! Enchanté.", "J'ai entendu parler de vos récentes fiançailles.", "Vous venez donc ici dans le but de vous marier ?", "Bien. Tout est prêt, nous pouvons commencer la cérémonie..."), DialogType.POSTDIALOG, new Runnable(){
												...
												public void run() {
													for(Player player : Bukkit.getOnlinePlayers()){
														if(!player.equals(p) && !player.equals(pp) && player.getLocation().distanceSquared(villager.getLocation()) <= 64){
															PNJDialog dialog = new PNJDialog(player, instance, villager, speech, DialogType.PREDIALOG, null, false, false);
															dialog.start();
														}
													}
													
													PNJDialog dialog2 = new PNJDialog(pp, instance, villager, speech, DialogType.PREDIALOG, new Runnable(){
									...
									new PNJDialog(p, this, villager, Arrays.asList("Silence, nous sommes en pleine cérémonie de mariage !"), DialogType.NONDIALOG, new Runnable(){
					...
					new PNJDialog(p, this, villager, Arrays.asList("La présence de %companion est nécessaire !"), DialogType.NONDIALOG, new Runnable(){
					...
					new PNJDialog(p, this, villager, Arrays.asList("Je n'aime pas briser un coeur, mais...", "%companion n'a pas encore accepté votre demande en mariage."), DialogType.NONDIALOG, new Runnable(){
				...
				new PNJDialog(p, this, villager, Arrays.asList("Je suis ici pour réaliser le mariage de vos rêves.", "Revenez me voir lorsque vous serez fiancé à quelqu'un."), DialogType.NONDIALOG, new Runnable(){
