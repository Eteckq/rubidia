...
public abstract class PNJHandler {
	...
	public enum PNJType {//ORDER MATTERS !!!!!!!! needs to be accorded with list of villager types (see minecraft wiki)
		SHOP(Profession.FARMER),
		INHABITANT(Profession.FARMER),
		QUEST(Profession.LIBRARIAN),
		PASTOR(Profession.PRIEST),
		SMITH(Profession.BLACKSMITH),
		BANK(Profession.BUTCHER),
	...
	public void resetPathfinders(Villager pnj, boolean fake){
		...
		villager.goalSelector = new PathfinderGoalSelector(villager.getWorld().methodProfiler);
		villager.targetSelector = new PathfinderGoalSelector(villager.getWorld().methodProfiler);
	...
	public void setFix(boolean fix) {
		this.fix = fix;
		if(fix){
			this.getEntity().addPotionEffect(new PotionEffect(PotionEffectType.SLOW, 99999999, 255, true, false), true);
			TeleportHandler.teleport(this.getEntity(), this.getLocation());
		}else{
	...
	public void createFake(Player player){
		...
		for(ArmorStand tag : this.getTag().getDisplays())EntityHandler.hideEntity(player, tag);
		...
		for(Player p : Bukkit.getOnlinePlayers()){
			if(!p.equals(player)){
				...
				for(ArmorStand stand : tag.getDisplays())EntityHandler.hideEntity(p, stand);
			}
		}
