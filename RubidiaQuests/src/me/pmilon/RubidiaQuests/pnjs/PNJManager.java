		...
		if(Configs.getPNJConfig().contains("pnjs")){
			for(String uuid : Configs.getPNJConfig().getConfigurationSection("pnjs").getKeys(false)){
				...
				if(type.equals(PNJType.QUEST)){
					...
					this.spawn(new ShopPNJ(uuid, title, name, location, age, fix, shop, Configs.getPNJConfig().getString(path + ".dialog")));
				}else if(type.equals(PNJType.BANK)){
					this.spawn(new BankPNJ(uuid, name, location, age, fix, Configs.getPNJConfig().getString(path + ".dialog")));
				}else if(type.equals(PNJType.SMITH)){
					this.spawn(new SmithPNJ(uuid, name, location, age, fix, Configs.getPNJConfig().getString(path + ".dialog")));
