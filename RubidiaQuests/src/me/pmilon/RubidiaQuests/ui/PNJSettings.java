...
public class PNJSettings extends UIHandler {
		...
		if(this.getMessage() != null){
			if(!this.getMessage().isEmpty()){
				if(this.getListeningId() == this.LISTENING_ID_NAME){
					PNJSession session = PNJManager.pnjs.get(this.getPnj().getEntityUUID().toString());
					PNJManager.pnjs.remove(this.getPnj().getEntityUUID().toString());
					this.getPnj().setName(this.getMessage());
					PNJManager.pnjs.put(this.getPnj().getEntityUUID().toString(), session);
					...
					else if(this.getPnj() instanceof ActivePNJ)((ActivePNJ)this.getPnj()).setDialog(MessageManager.filterDialog(this.getMessage()));
		...
		Bukkit.getScheduler().runTaskLater(QuestsPlugin.instance, new Runnable(){//to avoid entity nullException on move (delayed teleportation)
			public void run(){
				...
				if(getPnj() instanceof PastorPNJ){
					...
					if(!(getPnj() instanceof ActivePNJ) || getPnj().getType().equals(PNJType.SHOP))getMenu().setItem(SLOT_DIAL_QUST, getDialQust());
					if(getPnj().getType().equals(PNJType.QUEST) || getPnj() instanceof ActivePNJ)getMenu().setItem(SLOT_NO_QUEST, getNoQuestDial());
				}
	...
	private ItemStack getNoQuestDial(){
		...
		}else if(this.getPnj() instanceof ActivePNJ){
			meta.setDisplayName("Pre dialog");
			meta.setLore(Arrays.asList(((ActivePNJ)this.getPnj()).getDialog()));
		}
