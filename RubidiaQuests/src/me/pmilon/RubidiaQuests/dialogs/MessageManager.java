...
public class MessageManager {
	...
	public static String pnjDialog(PNJHandler pnj, DialogType type, Player p, String name, String dialog){
		...
		if(dialog.contains("%yes"))p.playSound(p.getLocation(), Sound.ENTITY_VILLAGER_YES, 1, pitch);
			...
			else if(type.equals(DialogType.POSTDIALOG))p.playSound(p.getLocation(), Sound.ENTITY_VILLAGER_YES, 1, pitch);
			else if(type.equals(DialogType.NONFINISHEDDIALOG))p.playSound(p.getLocation(), Sound.ENTITY_VILLAGER_NO, 1, pitch);
			else if(type.equals(DialogType.NONDIALOG))p.playSound(p.getLocation(), Sound.ENTITY_VILLAGER_AMBIENT, 1, pitch);
			else if(type.equals(DialogType.REQUIRED_DIALOG))p.playSound(p.getLocation(), Sound.ENTITY_VILLAGER_NO, 1, pitch);
		}
		String guildName = "";
		if(member.hasGuild()){
