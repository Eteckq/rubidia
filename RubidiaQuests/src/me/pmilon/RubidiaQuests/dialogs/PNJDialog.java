...
public class PNJDialog {
	...
	public void start(){
		...
		tpTask = new BukkitTask(QuestsPlugin.instance){
			...
			public void run() {
				if(getTalker() != null){
					getPlayer().teleport(getPlayer().getLocation().clone().setDirection((getTarget() != null ? getTarget().clone() : getTalker().getLocation().clone().add(0,((CraftEntity)getTalker()).getHandle().length*.9,0)).subtract(getPlayer().getEyeLocation()).toVector()));
				}
				if(isBlind()){
					if(!getPlayer().hasPotionEffect(PotionEffectType.BLINDNESS)){
						getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 99999999, 444, true, false), true);
					}
